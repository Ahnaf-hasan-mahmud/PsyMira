...
import HomecomingSceneArt from "@/components/story/HomecomingSceneArt";
import HomecomingReport from "@/components/story/HomecomingReport";
...
export default function HomecomingPage() {
  const sceneMap = useMemo(
    () => Object.fromEntries(HOMECOMING_ASSESSMENT.scenes.map((s) => [s.id, s])),
    []
  );

  const [currentId, setCurrentId] = useState(HOMECOMING_ASSESSMENT.start);
  ...
  const restart = useCallback(() => {
    busy.current = false;
    setCurrentId(HOMECOMING_ASSESSMENT.start);
    setPicks([]);
    setEcho("");
    setPhase("story");
  }, []);
  ...
  if (phase === "summary" || !current) {
    return <HomecomingReport picks={picks} onRestart={restart} />;
  }

  const step = picks.length + (phase === "echo" ? 1 : 0);
  const progress = Math.min(100, (step / HOMECOMING_ASSESSMENT.total) * 100);
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className={styles.art}>
              <HomecomingSceneArt scene={current.scene} />
            </div>
