...
export default function AssessmentReport({
  ...
  const report = useMemo(() => buildReport(picks), [picks]);
  ...
  // Persist once (no-op until Supabase is configured).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await supabase.from("reflections").insert({
        user_id: user.id,
        story_id: ASSESSMENT.id,
        story_title: ASSESSMENT.title,
        ...
        <div className={styles.topGrid}>
          ...
          <motion.section className={`${styles.card} ${styles.radarCard}`} variants={fadeUp}>
            ...
            <p className={styles.radarNote}>
              Eleven quiet patterns, mapped from the choices you made across the day.
            </p>
          </motion.section>
        </div>
